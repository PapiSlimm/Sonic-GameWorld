...
import {
  Activity, AlertTriangle, Building2, Flag, LayoutDashboard, LogOut, Package, ShieldAlert, Users, Wallet,
} from 'lucide-react';
import { Badge, Button, Input, useToast } from '@sonic-gameworld/ui';
import { getClient, API_BASE_URL } from '../lib/client.js';
...
const NAV = [
  ...
  { href: '/fraud', label: 'Fraud Signals', icon: AlertTriangle },
  ...
  return (
    <div className="flex min-h-screen">
      <aside className="hidden w-60 flex-col border-r border-border bg-panel/60 px-3 py-4 md:flex">
        ...
        <div className="px-2 pt-4 text-[10px] text-muted">Admin Console · Sonic GameWorld OS</div>
      </aside>
...
function TopBar() {
  ...
  const [email, setEmail] = useState('admin@sonicgameworld.dev');
  ...
  const signIn = async () => {
    setPending(true);
    try {
      const res = await getClient().auth.dev({ email, displayName: 'Admin Console' });
  ...
  return (
    <header className="flex flex-wrap items-center justify-between gap-3 border-b border-border bg-panel/40 px-4 py-3 md:px-6">
      <div className="flex items-center gap-2">
        <h1 className="text-sm font-semibold text-text">GameWorld Admin</h1>
      ...
      <div className="flex items-center gap-2">
        {session ? (
          ...
          <>
            <Input value={email} onChange={(e) => setEmail(e.target.value)} className="h-8 w-56 text-xs" placeholder="admin@sonicgameworld.dev" />
