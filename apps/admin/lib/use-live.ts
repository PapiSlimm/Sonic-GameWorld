...
/**
 * Attempts a live API call; on any failure (offline API, auth not configured, endpoint not yet
 * implemented on this deployment) it transparently falls back to the given demo dataset so every
 * admin screen renders a complete, believable UI with zero backend dependency. `source` tells the
